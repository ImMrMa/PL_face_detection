...
def agnex_ct_decode(t_heat,
    ...
    detections = torch.cat([
        bboxes, scores, t_xs, t_ys, l_xs, l_ys, b_xs, b_ys, r_xs, r_ys, clses
    ],
                           dim=2)
...
def ddd_decode(heat, rot, depth, dim, wh=None, reg=None, K=40):
    ...
    if wh is not None:
        ...
        detections = torch.cat([xs, ys, scores, rot, depth, dim, wh, clses],
                               dim=2)
    else:
...
def fadet_decode(heat, wh, reg=None, cat_spec_wh=False, p=0.1, large=False):
    ...
    if large:
        ...
        efficient_index=(wh[:,0]*wh[:,1])**0.5<=128
    if large:
        xs=xs*4
        ys=ys*4
    wh=wh[efficient_index]
    clses=clses.view(-1,1)
    scores=scores.view(-1,1)
    xs=xs.view(-1,1)
    ys=ys.view(-1,1)
    clses=clses[efficient_index]
    scores=scores[efficient_index]
    xs=xs[efficient_index]
    ys=ys[efficient_index]
    clses = clses.view(batch, -1, 1).float()
    scores = scores.view(batch,-1, 1)
    xs=xs.view(batch,-1,1)
    ys=ys.view(batch,-1,1)
    bboxes = torch.cat([
        xs - wh[..., 0:1] / 2, ys - wh[..., 1:2] / 2, xs + wh[..., 0:1] / 2,
        ys + wh[..., 1:2] / 2
    ],dim=2)           
