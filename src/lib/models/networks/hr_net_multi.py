...
class HighResolutionNet(nn.Module):
    ...
    def __init__(self, config, num_layers,heads,head_conv):
        extra = config
        self.heads=heads
            ...
            if head_conv > 0:
                ...
                if 'wh' in head:
                    if 'small' in head:                   
                        ...
                        channels=[64,32,32,8]
                    fcs = nn.ModuleList()
                    for i,channel in zip(dilations,channels):
                    ...
                    fcs.append(nn.Sequential(
                            nn.Conv2d(sum(channels), head_conv,
        ...
        self.inplanes=256
        self.trans_layers = self._make_trans_layer(
            2,
            [128, 64],
            [3, 3],
        )
    ...
    def _make_deconv_layer(self, num_layers, num_filters, num_kernels):
        ...
        return nn.Sequential(*layers)
    def _make_trans_layer(self, num_layers, num_filters, num_kernels):
        assert num_layers == len(num_filters), \
            'ERROR: num_deconv_layers is different len(num_deconv_filters)'
        assert num_layers == len(num_kernels), \
            'ERROR: num_deconv_layers is different len(num_deconv_filters)'
    ...
    def forward(self, x):
        ...
        x_trans=self.trans_layers(x_last)
        ...
        for head in self.heads:
            if 'small' in head:
                ...
                feature_layer=x_trans
            else:
                print('error')
            middle_layers=[]
            model_aspps=self.__getattr__(head)[:-1]
            fuse=self.__getattr__(head)[-1]
            for model_aspp in model_aspps:
                middle_layers.append(model_aspp(feature_layer))
            middle_layer=torch.cat(middle_layers,1)
            ret[head]=fuse(middle_layer)
        return [ret]
