...
def get_data_wider(ped_data, C, batchsize=8):
    ...
    while True:
        ...
        y_seman_batch = np.concatenate(y_seman_batch, axis=0)
        y_height_batch = np.concatenate(y_height_batch, axis=0)
        x_img_tensor=torch.tensor(np.copy(x_img_batch/255))[...,(2,1,0)].permute(0,3,1,2)
        x_img_tensor=(x_img_tensor-mean)/std
        if C.offset:
            y_offset_batch = np.concatenate(y_offset_batch, axis=0)
        current_ped += batchsize
        if C.offset:
            yield dict(input=x_img_tensor,hm=torch.tensor(np.copy(y_seman_batch)).float().permute(0,3,1,2),wh=torch.tensor(np.copy(y_height_batch)).float().permute(0,3,1,2), offset=torch.tensor(np.copy(y_offset_batch)).float().permute(0,3,1,2))
        else:
            yield np.copy(x_img_batch), [np.copy(y_seman_batch), np.copy(y_height_batch)]
